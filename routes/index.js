...
router.get('/', function(req, res, next) {
  res.render('index', { title: 'PlaylistManager' });
});
...
router.post("/register", (req, res, next) => {
  ...
  User.register(
    ...
    (err, newUser) => {
      // if successful then log user in and redirect to playlists
      req.login(newUser, (err) => { res.redirect("/playlists"); });
    } // Callback function
  );
});
...
router.get('/github/callback',
  ...
  (req, res, next) => { res.redirect('/playlists') }
);
